namespace UglyToad.PdfPig.Tests.ContentTests
{
    ...
    public class IccBasedAlternateColorSpaceTests
    {
        ...
        /// <summary>
        /// A type 2 tint transform running from all zeroes to <paramref name="c1"/>, which is as much of a
        /// function as a colour space needs to be built at all.
        /// </summary>
        private static DictionaryToken Tint(params double[] c1)
            => new DictionaryToken(new Dictionary<NameToken, IToken>
            {
                ...
                { NameToken.C0, new ArrayToken(c1.Select(_ => (IToken)new NumericToken(0)).ToArray()) },
                { NameToken.C1, new ArrayToken(c1.Select(x => (IToken)new NumericToken(x)).ToArray()) },
        ...
        private static ArrayToken SeparationOverCmykArray()
            => new ArrayToken(new IToken[]
            {
                NameToken.Separation, NameToken.Create("Spot"), NameToken.Devicecmyk, Tint(0, 0, 0, 1)
            });
        ...
        private static ICCBasedColorSpaceDetails Parse(IToken? alternate, int n,
            ...
            => Assert.IsType<ICCBasedColorSpaceDetails>(
                ParseAnyDetails(alternate, n, scanner, resources, trailingArrayElements));

        private static ColorSpaceDetails ParseAnyDetails(IToken? alternate, int n,
            TestPdfTokenScanner? scanner = null, DictionaryToken? resources = null,
            IToken[]? trailingArrayElements = null)
        {
            ...
            return store.GetColorSpaceDetails(NameToken.Iccbased,
                new DictionaryToken(new Dictionary<NameToken, IToken>
                {
        ...
        [Fact]
        public void AlternateOfAnIccBasedUsedAsAnIndexedBase_IsStillUsed()
        {
            // [ /Indexed [ /ICCBased <stream /N 3 /Alternate [/CalRGB ...]> ] 1 <...> ]. Reaching the
            // ICCBased space as the base of another space does not make its /Alternate any less the thing
            // that decides the colour, so the alternate has to survive the nesting.
            var profileStream = new StreamToken(
                new DictionaryToken(new Dictionary<NameToken, IToken>
                {
                    { NameToken.N, new NumericToken(3) },
                    { NameToken.Alternate, CalRgbArray() }
                }),
                new byte[] { 0x01 });
            ...
            var indexed = new ArrayToken(new IToken[]
            {
                NameToken.Indexed,
            ...
            var details = store.GetColorSpaceDetails(NameToken.Indexed,
                new DictionaryToken(new Dictionary<NameToken, IToken>
                {
                    { NameToken.ColorSpace, indexed }
                }));

            var asIndexed = Assert.IsType<IndexedColorSpaceDetails>(details);
            var icc = Assert.IsType<ICCBasedColorSpaceDetails>(asIndexed.BaseColorSpace);

            Assert.Equal(ColorSpace.CalRGB, icc.AlternateColorSpace.Type);
        }

        [Fact]
        public void AlternateCyclingBackThroughADeviceN_Terminates()
        {
            // The prohibition on an ICCBased alternate is only checked on the alternate itself, so a cycle
            // that goes the long way round is not caught by it: /Alternate 7 0 R, where object 7 is a
            // DeviceN whose own alternate is [/ICCBased 9 0 R] and stream 9 points back at 7 0 R. Neither
            // DeviceN nor Separation refuses to recurse, and an ICCBased space reached as another space's
            // alternate still resolves an alternate of its own, so what ends the chain is the parser's
            // limit on how deeply colour spaces may nest.
            //
            // NB: as above, a regression here kills the test host rather than failing an assertion.
            var scanner = new TestPdfTokenScanner();

            var deviceNReference = new IndirectReference(7, 0);
            var innerArrayReference = new IndirectReference(8, 0);
            var innerStreamReference = new IndirectReference(9, 0);

            scanner.Objects[deviceNReference] = new ObjectToken(XrefLocation.File(0), deviceNReference,
                new ArrayToken(new IToken[]
                {
                    NameToken.Devicen,
                    new ArrayToken(new IToken[]
                    ...
                    new IndirectReferenceToken(innerArrayReference),
                    Tint(1, 1, 1)
                }));

            scanner.Objects[innerArrayReference] = new ObjectToken(XrefLocation.File(0), innerArrayReference,
                new ArrayToken(new IToken[]
                {
                    NameToken.Iccbased, new IndirectReferenceToken(innerStreamReference)
                }));

            scanner.Objects[innerStreamReference] = new ObjectToken(XrefLocation.File(0), innerStreamReference,
                new StreamToken(
                    new DictionaryToken(new Dictionary<NameToken, IToken>
                    {
                        { NameToken.N, new NumericToken(3) },
                        { NameToken.Alternate, new IndirectReferenceToken(deviceNReference) }
                    }),
                    new byte[] { 0x01 }));

            var details = Parse(new IndirectReferenceToken(deviceNReference), n: 3, scanner);

            // The DeviceN takes three components like the space itself, so it survives the width check and
            // every level the budget allows is kept. Walk the alternates down to the end of the chain
            // rather than pinning the exact number of them, which is the budget's to decide.
            ColorSpaceDetails current = details;
            var levels = 0;
            ...
            while (levels < 100)
            {
                ...
                {
                    ICCBasedColorSpaceDetails icc => icc.AlternateColorSpace,
                    DeviceNColorSpaceDetails deviceN => deviceN.AlternateColorSpace,
                    _ => null
                };
            ...
            // Both spaces in the cycle were built at least once, and the chain ended well short of the
            // walk's own limit rather than running on.
            Assert.InRange(levels, 2, 99);

            // Whichever of the two the budget stops on falls back to the space implied by /N.
            Assert.Same(DeviceRgbColorSpaceDetails.Instance, current);
        }

        [Fact]
        public void SeparationAlternateCyclingBackToItself_Terminates()
        {
            // Nothing in a Separation refuses to resolve its own alternate, so [/Separation /Spot 10 0 R
            // <tint>] whose alternate is that very array is a cycle with no ICCBased space in it at all.
            // Only the nesting budget ends it.
            //
            // NB: as above, a regression here kills the test host rather than failing an assertion.
            var scanner = new TestPdfTokenScanner();
            var separationReference = new IndirectReference(10, 0);

            scanner.Objects[separationReference] = new ObjectToken(XrefLocation.File(0), separationReference,
                new ArrayToken(new IToken[]
                {
                    NameToken.Separation,
                    NameToken.Create("Spot"),
                    new IndirectReferenceToken(separationReference),
                    Tint(1)
                }));
            ...
            var details = store.GetColorSpaceDetails(NameToken.Separation,
                new DictionaryToken(new Dictionary<NameToken, IToken>
                {
                    { NameToken.ColorSpace, new IndirectReferenceToken(separationReference) }
                }));

            var separation = Assert.IsType<SeparationColorSpaceDetails>(details);

            ColorSpaceDetails current = separation;
            var levels = 0;

            while (current is SeparationColorSpaceDetails inner && levels < 100)
            {
                current = inner.AlternateColorSpace;
                levels++;
            }

            Assert.InRange(levels, 1, 99);
            Assert.IsType<UnsupportedColorSpaceDetails>(current);
        }
        ...
        [InlineData(2)]
        [InlineData(5)]
        public void AComponentCountNoIccBasedSpaceMayHave_IsUnsupportedRatherThanThrown(int n)
        {
            // The colour space constructor throws on an /N outside 1, 3 and 4 - there is nothing to build a
            // space from - and nothing between the parser and the caller of GetPage catches it, so the
            // parser has to recognise the value itself and answer the way its other rejections do.
            var details = ParseAnyDetails(NameToken.Devicergb, n);

            Assert.IsType<UnsupportedColorSpaceDetails>(details);
        }
        ...
        public void DefaultColorSpaceSubstitutionStillAppliesToASeparationAlternate()
        {
            ...
            {
                NameToken.Separation, NameToken.Create("Spot"), NameToken.Devicergb, Tint(1, 1, 1)
            });
