namespace UglyToad.PdfPig.Tests.ContentTests
{
    ...
    /// fallback detail. PDFBox accepts a name or an array (PDICCBased.getAlternateColorSpace); PdfPig used to
    /// accept only a direct name and silently ignore everything else.
    /// </para>
    /// </summary>
    public class IccBasedAlternateColorSpaceTests
    {
        ...
        {
            // /Alternate [ /CalRGB << /WhitePoint [...] >> ]. Previously ignored outright, which silently
            // downgraded the colour space to the DeviceRGB implied by /N.
        ...
        {
            // /Alternate 5 0 R -> /DeviceCMYK. The old lookup passed no scanner, so an indirect alternate
            // was missed just as an array one was.
        ...
        public void AlternateIsNotSubjectToTheDefaultColorSpaceSubstitution()
        {
            ...
            // selected that way, so /DefaultCMYK must not capture it - PDFBox reaches the same result by
            // resolving the alternate with no resources at all.
