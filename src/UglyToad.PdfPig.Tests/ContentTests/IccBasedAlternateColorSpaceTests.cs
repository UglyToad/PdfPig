namespace UglyToad.PdfPig.Tests.ContentTests
{
    ...
    public class IccBasedAlternateColorSpaceTests
    {
        ...
        public void AlternateCyclingBackThroughADeviceN_Terminates()
        {
            ...
            // DeviceN nor Separation refuses to recurse, so the chain only ends because an ICCBased space
            // reached as another space's alternate does not resolve an alternate of its own.
            //
            ...
            // the whole chain is kept - only the innermost ICCBased falls back to the space implied by /N.
            var deviceN = Assert.IsType<DeviceNColorSpaceDetails>(details.AlternateColorSpace);
            var inner = Assert.IsType<ICCBasedColorSpaceDetails>(deviceN.AlternateColorSpace);

            Assert.Same(DeviceRgbColorSpaceDetails.Instance, inner.AlternateColorSpace);
        }
