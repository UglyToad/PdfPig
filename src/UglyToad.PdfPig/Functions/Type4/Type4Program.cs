namespace UglyToad.PdfPig.Functions.Type4
{
    ...
    internal sealed class Type4Program
    {
        ...
        /// <summary>
        /// Executes the program. Nested procedures are executed iteratively.
        /// </summary>
        public void Execute(ref OperandStack stack)
        {
            Span<CallFrame> initialBuffer = stackalloc CallFrame[32];
            var frames = new CallStack(initialBuffer);
            ...
            while (true)
            {
                ...
                while (i < code.Length)
                {
                    // The instruction pointer is advanced before the instruction runs so that the
                    // frame pushed by a call resumes at the following instruction.
                    ...
                    switch (instruction.Op)
                    {
                        ...
                        case OpCode.If:
                        {
                            ...
                            if (condition)
                            {
                                frames.Push(procedureIndex, i);
                                procedureIndex = procedure;
                                i = 0;
                                code = procedures[procedureIndex];
                            }
                            break;
                        }
                        ...
                        case OpCode.IfElse:
                        {
                            ...
                            frames.Push(procedureIndex, i);
                            procedureIndex = condition ? procedure1 : procedure2;
                            i = 0;
                            code = procedures[procedureIndex];
                            break;
                        }
                ...
                // (mirrors the tail of the previous InstructionSequence.Execute). The instruction
                // pointer stays at the end of the procedure, so the check is repeated every time
                // control returns here, as it was when this was a loop around a recursive call.
                if (stack.Count > 0 && stack.Peek().Kind == OperandKind.Procedure)
                {
                    frames.Push(procedureIndex, i);
                    procedureIndex = stack.Pop().AsProcedureIndex;
                    i = 0;
                    continue;
                }
                ...
                frames.Pop(out procedureIndex, out i);
            }
        }
