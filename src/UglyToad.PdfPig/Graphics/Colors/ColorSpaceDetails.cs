namespace UglyToad.PdfPig.Graphics.Colors
{
    ...
    public abstract class ColorSpaceDetails
    {
        ...
        /// <param name="g">The green component, in [0, 1].</param>
        /// <param name="b">The blue component, in [0, 1].</param>
        public virtual void GetRgb(ReadOnlySpan<double> values, RenderingIntent intent, out double r, out double g, out double b)
            => GetRgb(values, out r, out g, out b);
        ...
        /// <summary>
        /// Map component values normalised to [0, 1] into this colour space's
        /// native input range. Two callers share this scaling because they
        /// coincide: indexed colour-table bytes (ISO 32000-1 §8.6.6.3, each
        /// scaled to the range of the corresponding base component) and the
        /// default image Decode (§8.9.5.2). The default is the identity, which
        ...
        /// </summary>
        internal virtual double[] DenormalizeComponents(double[] normalized) => normalized;
        ...
        /// </summary>
        private protected static void GetRgbViaTint(PdfFunction tint, ColorSpaceDetails alternate, RenderingIntent intent,
            ReadOnlySpan<double> tintInput, out double r, out double g, out double b)
        {
            ...
            alternate.GetRgb(buffer.Slice(0, written), intent, out r, out g, out b);
        }
    }
    ...
    public sealed class IndexedColorSpaceDetails : ColorSpaceDetails
    {
        ...
        internal override double[] Process(double[] values, RenderingIntent intent)
        {
            ...
            return BaseColorSpace.Process(BaseColorSpace.DenormalizeComponents(scaledCsBytes), intent);
        }
        ...
        public override IColor GetColor(params double[] values)
        {
            ...
            return cache.GetOrAdd(values[0], v =>
            {
                ...
                return BaseColorSpace.GetColor(BaseColorSpace.DenormalizeComponents(scaledCsBytes));
            });
        }
        ...
        internal override IColor GetColor(double[] values, RenderingIntent intent)
        {
            ...
            return BaseColorSpace.GetColor(BaseColorSpace.DenormalizeComponents(scaledCsBytes), intent);
        }
        ...
        public override void GetRgb(ReadOnlySpan<double> values, RenderingIntent intent, out double r, out double g, out double b)
        {
            ...
            BaseColorSpace.GetRgb(buffer.Slice(0, components), intent, out r, out g, out b);
        }
    ...
    public sealed class DeviceNColorSpaceDetails : ColorSpaceDetails
    {
        ...
        public override void GetRgb(ReadOnlySpan<double> values, RenderingIntent intent, out double r, out double g, out double b)
        {
            GetRgbViaTint(TintFunction, AlternateColorSpace, intent, values, out r, out g, out b);
        }
    ...
    public sealed class SeparationColorSpaceDetails : ColorSpaceDetails
    {
        ...
        public override void GetRgb(ReadOnlySpan<double> values, RenderingIntent intent, out double r, out double g, out double b)
        {
            GetRgbViaTint(TintFunction, AlternateColorSpace, intent, values.Slice(0, 1), out r, out g, out b);
        }
    ...
    public sealed class LabColorSpaceDetails : ColorSpaceDetails
    {
        ...
        internal override Span<byte> Transform(Span<byte> decoded)
        {
            ...
            for (var i = 0; i < decoded.Length; i += 3)
            {
                ...
                var lab = DenormalizeComponents([decoded[i] / 255.0, decoded[i + 1] / 255.0, decoded[i + 2] / 255.0]);
                GetRgb(lab, out double r, out double g, out double b);
                transformed[index++] = ConvertToByte(r);
                transformed[index++] = ConvertToByte(g);
                transformed[index++] = ConvertToByte(b);
            }
        ...
        /// <inheritdoc/>
        internal override double[] DenormalizeComponents(double[] normalized)
        {
            // Each normalised component in [0, 1] is scaled to its native range.
            // For Lab that is L* in [0, 100] and a*/b* in the space's Range
            // (Matrix = [a_min a_max b_min b_max]). This matches both the indexed
            // colour-table scaling (ISO 32000-1 §8.6.6.3) and the default image
            // Decode (§8.9.5.2), which coincide for Lab.
            return
            [
                normalized[0] * 100.0,
    ...
    public sealed class ICCBasedColorSpaceDetails : ColorSpaceDetails
    {
        ...
        public override void GetRgb(ReadOnlySpan<double> values, RenderingIntent intent, out double r, out double g, out double b)
        {
            if (useProfile)
            {
                ...
                if (t is not null)
                {
                    (r, g, b) = t.ToRgb(values);
                    return;
                }
            }
            
            int n = NumberOfColorComponents;
            Span<double> clipped = n <= 32 ? stackalloc double[n] : new double[n];
            for (int c = 0; c < n; c++)
            {
                int i = 2 * c;
